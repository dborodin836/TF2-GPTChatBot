...
def test_deny_empty(setup_mocks):
    ...
    # Test
    logline = LogLine(username="user1", player=player_1, is_team_message=False, prompt="!test TEST PROMPT")
...
def test_admin_only(setup_mocks):
    ...
    # Test
    logline = LogLine(username="admin", player=admin_player, is_team_message=False, prompt="!test TEST PROMPT")
    controller.process_line(logline)
    assert spy.call_count == 1
...
def test_moderation(setup_mocks, mocker):
    ...
    logline_1 = LogLine(username="user1", player=player_1, is_team_message=False, prompt="!test TEST PROMPT")
    controller.process_line(logline_1)
    assert spy.call_count == 1
...
def test_empty_prompt_response(setup_mocks):
    ...
    class TestCmd(LLMChatCommand):
        provider = DummyProvider
        wrappers = [
            traits.empty_prompt_message_response("EMPTY RESPONSE")
        ]
