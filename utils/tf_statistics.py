...
MELEE_WEAPONS_KILL_IDS = {"skullbat", "nonnonviolent_protest", "crossing_guard", "disciplinary_action", "memory_maker",
                          ...
                          "tribalkukri", "big_earner", "black_rose", "kunai", "knife", "sharp_dresser", "spy_cicle"}
...
class StatsData:
    ...
    @classmethod
    def process_kill(cls, killer_username: str, victim_username: str, weapon: str, is_crit: bool) -> None:
        print(f"'{killer_username}' killed '{victim_username} with {weapon} {'crit' if is_crit else ''}'")
    ...
    def _get_steam_ban_data(cls, steamids64) -> List[dict]:
        ...
        try:
            response = requests.get(
                f" http://api.steampowered.com/ISteamUser/GetPlayerBans/v1/?key={config.STEAM_WEBAPI_KEY}&steamids={','.join(steamids64)}").json()
